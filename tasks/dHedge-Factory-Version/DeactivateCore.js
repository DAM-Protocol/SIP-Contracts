...
task("DeactivateCore", "Task to deactivate or reactivate a core contract")
  ...
  .addParam(
    "deactivate",
    "Boolean representing deactivating the contract or reactivating it",
    true,
    types.boolean
  )
  .setAction(async (taskArgs) => {
    ...
    try {
      ...
      console.info(
        `Core contract ${
          taskArgs.deactivate ? "deactivate" : "reactivate"
        } complete !`
      );
    } catch (error) {
      console.info(
        `${error.message} while ${
          taskArgs.pause ? "deactivating" : "reactivating"
        } the core contract at ${taskArgs.core}`
      );
    }
  });
